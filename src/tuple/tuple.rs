...
impl Tuple {
    fn add(first: &Tuple, second: &Tuple) -> Tuple {
        Tuple {
            x: first.x + second.x,
            y: first.y + second.y,
            z: first.z + second.z,
    ...
    fn subtract(first: &Tuple, second: &Tuple) -> Tuple {
        Tuple {
            x: first.x - second.x,
            y: first.y - second.y,
            z: first.z - second.z,
    ...
    fn scale(tuple: &Tuple, factor: f64) -> Tuple {
        Tuple {
            x: factor * tuple.x,
            y: factor * tuple.y,
            z: factor * tuple.z,
    ...
    fn dot_product(first: &Tuple, second: &Tuple) -> f64 {
        first.x * second.x + first.y * second.y + first.z * second.z + first.w * second.w
    }

    fn cross_product(first: &Tuple, second: &Tuple) -> Tuple {
        Tuple {
...
mod tests {
    ...
    #[test]
    fn test_point() {
        let test_point = point(0.0, 1.0, -1.0);
    ...
    #[test]
    fn test_vector() {
        let test_vector = vector(0.0, 1.0, -1.0);
        let test_tuple = Tuple {
            x: 0.0,
            y: 1.0,
            z: -1.0,
    ...
    #[test]
    fn test_add_tuple() {
        let test_point = point(3.0, -2.0, 5.0);
        let test_vector = vector(-2.0, 3.0, 1.0);
        let result = Tuple::add(&test_point, &test_vector);
        let actual_result = point(1.0, 1.0, 6.0);
        assert_eq!(result, actual_result)
    }

    #[test]
    fn test_subtract_points() {
        let test_point_1 = point(3.0, 2.0, 1.0);
        let test_point_2 = point(5.0, 6.0, 7.0);
        let result = Tuple::subtract(&test_point_1, &test_point_2);
        let actual_result = vector(-2.0, -4.0, -6.0);
        assert_eq!(result, actual_result)
    }

    #[test]
    fn test_subtract_vector_point() {
        let test_point = point(3.0, 2.0, 1.0);
        let test_vector = vector(5.0, 6.0, 7.0);
        let result = Tuple::subtract(&test_point, &test_vector);
        let actual_result = point(-2.0, -4.0, -6.0);
        assert_eq!(result, actual_result)
    }

    #[test]
    fn test_subtract_vectors() {
        let test_vector_1 = vector(3.0, 2.0, 1.0);
        let test_vector_2 = vector(5.0, 6.0, 7.0);
    ...
    fn test_vector_normalize() {
        let test_vector = vector(1.0, 2.0, 3.0);
        let result = test_vector.normalize();
        let actual_result = test_vector.magnitude();
        let actual_result = vector(
            1.0 / actual_result, 2.0 / actual_result, 3.0 / actual_result
        );
        assert_eq!(result, actual_result)
    }
    ...
    fn test_vector_dot_product() {
        let test_vector_1 = vector(1.0, 2.0, 3.0);
        let test_vector_2 = vector(2.0, 3.0, 4.0);
    ...
    fn test_vector_cross_product() {
        ...
        let actual_result = vector(-1.0, 2.0, -1.0);
        assert_eq!(result, actual_result);
